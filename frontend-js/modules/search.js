//axios is used to send asynchronous req to server
import axios from 'axios'

//to sanitize HTML in front end
...
export default class Search {
   ...
   constructor () {
       ...
       this.injectHTML()
        this.headerSearchIcon = document.querySelector(".header-search-icon")//class in header.js
       this.overlay=document.querySelector(".search-overlay")//class below
       this.closeIcon=document.querySelector(".close-live-search")//class below
       this.inputField=document.querySelector("#live-search-field")//id below
    ...
    //2. Events like clicking,scrolling
    events(){
    ...
    keyPressHandler(){
      ...
      if(value != "" && value != this.previousValue){
        clearTimeout(this.typingWaitTimer)
        this.showLoaderIcon()
        this.hideResultsArea()//for second search
    ...
    sendRequest(){
     ...
     axios.post('/search',{_csrf:this._csrf,searchTerm : this.inputField.value}).then(response=>{
      //response is servers response i.e json data to the browser which is an array
      console.log(response.data)// in browser
      this.renderResultsHTML(response.data)//just below
     }).catch(()=>{
       alert('sorry the req failed')
     })
    }

    renderResultsHTML(posts){
      if(posts.length){
        //for front end sanitize
    ...
    //overlay means search ko screen pop hune
    openOverlay(){
        this.overlay.classList.add("search-overlay--visible")
       setTimeout(()=>  this.inputField.focus(),50)//put the cursor there  after 50ms
    }
